...
class WatchedExportSerializerMixin(serializers.ModelSerializer):
    watchers = UserRelatedField(many=True, required=False)

    def __init__(self, *args, **kwargs):
        self._watchers_field = self.base_fields.pop("watchers", None)
        super(WatchedExportSerializerMixin, self).__init__(*args, **kwargs)
    ...
    def restore_object(self, attrs, instance=None):
        ...
        instance = super(WatchedExportSerializerMixin, self).restore_object(attrs, instance)
    ...
    def to_native(self, obj):
        ret = super(WatchedExportSerializerMixin, self).to_native(obj)
...
class MilestoneExportSerializer(WatchedExportSerializerMixin):
    owner = UserRelatedField(required=False)
    modified_date = serializers.DateTimeField(required=False)
...
class TaskExportSerializer(CustomAttributesValuesExportSerializerMixin, HistoryExportSerializerMixin,
                           AttachmentExportSerializerMixin, WatchedExportSerializerMixin):
    owner = UserRelatedField(required=False)
...
class ProjectExportSerializer(WatchedExportSerializerMixin):
    owner = UserRelatedField(required=False)
